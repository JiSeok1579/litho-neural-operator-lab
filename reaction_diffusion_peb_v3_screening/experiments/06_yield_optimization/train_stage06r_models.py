"""Stage 06R -- train the feature-engineered surrogate stack:
  classifier + 4-target regressor + aux CD_fixed regressor +
  direct strict_score regressor.

Train protocol (mirrors Stage 06P)
    1. Reproduce the closed Stage 04C 80/20 split (seed=13). Hold
       out the same 1,074 04C test rows so 06P vs 06R numbers stay
       directly comparable on a fixed ground truth.
    2. Train on the 06R pool (14,194 rows) MINUS the 1,074 04C-test
       rows = ~13,120 train rows.
    3. Save 4 joblibs and one metrics JSON.

Feature set: 11 raw 06P features + 9 derived process-budget features
loaded from outputs/models/stage06R_feature_list.json. Each model's
joblib metadata records the same feature list so downstream code can
verify before predicting.

Joblib size guard: the 4-target regressor uses n_estimators=120 to
keep the joblib comfortably under GitHub's 100 MB push limit at this
training pool size with 20 input features.

Outputs
    outputs/models/stage06R_classifier.joblib
    outputs/models/stage06R_regressor.joblib                 (n=120)
    outputs/models/stage06R_aux_cd_fixed_regressor.joblib
    outputs/models/stage06R_strict_score_regressor.joblib    (n=200)
    outputs/logs/stage06R_model_metrics.json

Policy
    v2_OP_frozen           = true
    published_data_loaded  = false
    No external calibration. Closed Stage 04C / 04D / 06C / 06L / 06P
    joblibs are not modified.
"""
...
def main() -> int:
    ...
    # 04C CSV does NOT carry the derived columns (it predates 06R) --
    # augment in-memory so the holdout can be predicted by 06R models.
    rows_04c = augment_rows(rows_04c_raw)
    rows_06r_all = read_labels_csv(args.stage06r_csv)
    ...
    for r in rows_06r_all:
        ...
        if src.startswith("stage06H/stage06C/stage04C"):
            ...
            if key in test_keys:
                ...
                continue
        train_06r.append(r)
    print(f"  06R training pool: {len(train_06r)} rows  "
          f"(dropped {n_drop} 04C-test rows)")

    by_source = Counter(r.get("source", "?") for r in train_06r)
    by_mode   = Counter(r.get("mode", "?")   for r in train_06r)
    by_family = Counter(r.get("recipe_family", "?") for r in train_06r)
    ...
    # ----- Train -----
    X_tr = _build_X(train_06r, feature_keys)
    y_tr = [r.get("label", "?") for r in train_06r]
    X_te = _build_X(rows_04c_te, feature_keys)
    y_te = [r.get("label", "?") for r in rows_04c_te]

    print(f"\n  training 06R classifier (n_estimators={args.n_estimators})")
    clf = RandomForestClassifier(
        n_estimators=args.n_estimators, max_depth=None,
        random_state=args.seed_train, n_jobs=args.n_jobs,
    )
    clf.fit(X_tr, y_tr)
    save_model(clf, V3_DIR / "outputs" / "models" / "stage06R_classifier.joblib",
                metadata={"n_train": len(train_06r),
                          "feature_keys": feature_keys,
                          "raw_feature_keys": feat_payload["raw_feature_keys"],
                          "derived_feature_keys": feat_payload["derived_feature_keys"],
                          "labels": LABEL_ORDER, "stage": "06R"})

    Y_full = _build_Y(train_06r)
    finite_mask = np.all(np.isfinite(Y_full), axis=1)
    print(f"  training 06R 4-target regressor on {int(finite_mask.sum())} rows  "
    ...
    save_model(reg, V3_DIR / "outputs" / "models" / "stage06R_regressor.joblib",
    ...
    y_cd = np.array([_safe_float(r.get("CD_final_nm")) for r in train_06r])
    cd_finite = np.isfinite(y_cd)
    print(f"  training 06R aux CD_fixed regressor on {int(cd_finite.sum())} rows  "
    ...
    print(f"  training 06R STRICT_SCORE regressor on {int(strict_finite.sum())} rows  "
    ...
    metrics = {
        ...
        "split": {
            ...
            "note": ("06R trained on 06P pool (raw 06P features + 9 "
                      "derived process-budget features). Same 04C "
                      "1,074 holdout as 06L/06P for direct comparison. "
                      "Closed Stage 04C / 04D / 06C / 06L / 06P joblibs "
                      "are not modified."),
        },
        "feature_keys":            feature_keys,
        "raw_feature_keys":        feat_payload["raw_feature_keys"],
    ...
    (logs_dir / "stage06R_model_metrics.json").write_text(
        json.dumps(metrics, indent=2))

    # ----- Console summary -----
    print(f"\nStage 06R model metrics (test: 04C 1,074 holdout)")
    ...
    # Feature importance summary -- top 8.
    imp = list(zip(feature_keys, metrics["feature_importance_strict_score"]))
    imp.sort(key=lambda kv: -kv[1])
    print(f"  top-8 strict_score head feature importance (06R):")
    for k, v in imp[:8]:
        print(f"    {k:<24} {v:.4f}")
    print(f"  metrics -> {logs_dir / 'stage06R_model_metrics.json'}")
    return 0
