"""Stage 06L -- train the 4-head surrogate stack:
  classifier + 4-target regressor + aux CD_fixed regressor +
  *direct strict_score regressor*.

The strict_score head is the new piece of this stage. It is trained
on the per-row strict_score label written by
build_stage06l_dataset.py and is intended to be the single ranking
signal across Mode A and Mode B candidates.

Train protocol (mirrors Stage 06H to keep apples-to-apples)
    1. Reproduce the closed Stage 04C 80/20 split (seed=13). Hold
       out the same 1,074 04C test rows so 06L vs 06H numbers are
       comparable on a fixed ground truth.
    2. Train on the 06L pool (9,918 rows) MINUS the 1,074 04C-test
       rows = ~8,844 train rows.
    3. Save 4 joblibs and one metrics JSON.
    4. Per-mode evaluation (mode_a vs mode_b) on the test set.

Joblib size guard: the 4-target regressor is trained at
n_estimators=200 to stay under GitHub's 100 MB push limit on this
training pool size. Other heads stay at 300 trees -- they are smaller.

Outputs
    outputs/models/stage06L_classifier.joblib
    outputs/models/stage06L_regressor.joblib                    (n=200)
    outputs/models/stage06L_aux_cd_fixed_regressor.joblib
    outputs/models/stage06L_strict_score_regressor.joblib       (n=200)
...
def main() -> int:
    ...
    p.add_argument("--n_estimators_regressor", type=int, default=200,
                   help="4-target regressor only -- keeps joblib < 100 MB.")
    p.add_argument("--n_estimators_strict",    type=int, default=200,
                   help="strict_score head; 1D so usually small but pinned "
                        "consistent with regressor for parity.")
    ...
    rows_06l_all = read_labels_csv(args.stage06l_csv)
    ...
    train_06l: list[dict] = []
    n_drop = 0
    for r in rows_06l_all:
        if str(r.get("source", "")).startswith("stage06H/stage06C/stage04C"):
            ...
            if key in test_keys:
                ...
                continue
        train_06l.append(r)
    print(f"  06L training pool: {len(train_06l)} rows  "
          f"(dropped {n_drop} 04C-test rows)")

    by_source = Counter(r.get("source", "?") for r in train_06l)
    by_mode   = Counter(r.get("mode", "?")   for r in train_06l)
    print(f"  by source (training pool):")
    for k, v in by_source.most_common():
        print(f"    {k:<48} {v:>6}")
    print(f"  by mode (training pool):  {dict(by_mode)}")

    # ----- Train -----
    X_tr = _build_X(train_06l); y_tr = [r.get("label", "?") for r in train_06l]
    X_te = _build_X(rows_04c_te); y_te = [r.get("label", "?") for r in rows_04c_te]

    print(f"\n  training 06L classifier (n_estimators={args.n_estimators})")
    clf = RandomForestClassifier(
        n_estimators=args.n_estimators, max_depth=None,
        random_state=args.seed_train, n_jobs=args.n_jobs,
    )
    clf.fit(X_tr, y_tr)
    save_model(clf, V3_DIR / "outputs" / "models" / "stage06L_classifier.joblib",
                metadata={"n_train": len(train_06l), "feature_keys": FEATURE_KEYS,
                          "labels": LABEL_ORDER, "stage": "06L"})

    Y_full = _build_Y(train_06l)
    finite_mask = np.all(np.isfinite(Y_full), axis=1)
    print(f"  training 06L 4-target regressor on {int(finite_mask.sum())} rows  "
    ...
    save_model(reg, V3_DIR / "outputs" / "models" / "stage06L_regressor.joblib",
    ...
    y_cd = np.array([_safe_float(r.get("CD_final_nm")) for r in train_06l])
    cd_finite = np.isfinite(y_cd)
    print(f"  training 06L aux CD_fixed regressor on {int(cd_finite.sum())} rows  "
    ...
    print(f"  training 06L STRICT_SCORE regressor on {int(strict_finite.sum())} rows  "
    ...
    # strict_score per row on the test set: rows_04c_te come from the
    # closed Stage 04C dataset CSV which doesn't carry the
    # strict_score_per_row column (it was added by build_stage06l_dataset
    # only to the 06L training rows). Compute the label here using the
    # same Stage 06G strict config so the regressor can be evaluated.
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    from build_stage06l_dataset import per_row_strict_score   # noqa: E402
    strict_yaml = yaml.safe_load(
        (V3_DIR / "outputs" / "yield_optimization"
         / "stage06G_strict_score_config.yaml").read_text()
    )
    ...
    metrics = {
        ...
        "split": {
            ...
            "note": ("06L trained on 06H pool union 06J FD additions, "
                      "with the 04C held-out test set strictly removed. "
                      "Closed Stage 04C / 04D / 06C joblibs are not "
                      "modified."),
        },
        "feature_keys":       FEATURE_KEYS,
        "labels":             LABEL_ORDER,
        "regression_targets": REGRESSION_TARGETS,
        "n_train":            len(train_06l),
        ...
        "feature_importance_classifier": [float(v) for v in clf.feature_importances_],
        "feature_importance_strict_score": [float(v) for v in strict_reg.feature_importances_],
    }

    logs_dir = V3_DIR / "outputs" / "logs"
    logs_dir.mkdir(parents=True, exist_ok=True)
    (logs_dir / "stage06L_model_metrics.json").write_text(
        json.dumps(metrics, indent=2))

    # ----- Console summary -----
    print(f"\nStage 06L model metrics (test: 04C 1,074 holdout)")
    ...
    print(f"  metrics -> {logs_dir / 'stage06L_model_metrics.json'}")
    return 0
