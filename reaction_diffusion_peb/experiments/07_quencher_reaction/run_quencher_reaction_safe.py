"""PEB Phase-7 demo: acid-quencher reaction (safe kq regime).

Run:
    python reaction_diffusion_peb/experiments/07_quencher_reaction/run_quencher_reaction_safe.py

Sweeps ``k_q in {1, 5, 10} 1/s`` with the FD evolver. Reports per-kq:
...
Stays well below the stiff CFL bound — see the companion stiff demo
for kq in {100, 300, 1000}.
"""
...
def main() -> None:
    ...
    print(f"  kdep={KDEP}  kloss={KLOSS}  t_end={T_END_S} s")

    rows = [["kq_s_inv", "dt_max_s", "stiff_term",
             "H_peak", "H_mean",
             "Q_min", "Q_mean",
             "P_max", "P_mean",
    ...
    fig = show_quencher_kq_sweep(
        ...
        suptitle=(f"PEB phase 7 (safe kq): DH={DH}  DQ={DQ}  Q0={Q0}  "
                  f"kdep={KDEP}  kloss={KLOSS}  t={T_END_S} s"),
    )
    out_fig = save_figure(fig, OUT_FIG / "peb_phase7_quencher_safe_sweep.png")
    print(f"  wrote {out_fig}")

    fig = show_quencher_mass_budget(
        histories, KQ_VALUES,
        suptitle=f"PEB phase 7 (safe kq) — mass-budget identity check",
    )
    out_fig = save_figure(fig, OUT_FIG / "peb_phase7_quencher_safe_budget.png")
    print(f"  wrote {out_fig}")

    # CSV logs
    OUT_LOG.mkdir(parents=True, exist_ok=True)
    log_main = OUT_LOG / "peb_phase7_quencher_safe_metrics.csv"
    with open(log_main, "w", newline="") as f:
        csv.writer(f).writerows(rows)
    print(f"  wrote {log_main}")

    log_hist = OUT_LOG / "peb_phase7_quencher_safe_budget_history.csv"
